...
class NativeCPU:
    def __init__(self, library, threads=1):
        ...
        self.sparse.argtypes=[ctypes.c_int64]*4+[ctypes.c_float]*6+[ctypes.c_void_p]*16
    ...
    def arrivals(self, n):
        ...
        edges=torch.empty(count,dtype=torch.int64)
        self.arrivals_fn(*args,edges.data_ptr())
        return torch.zeros_like(edges),edges

    @torch.no_grad()
    def synchronize(self, rule):
        if rule.homeostatic_exponent.any():
            from .plasticity import Plasticity
            Plasticity.synchronize(rule)
            return
        self.sync_fn(rule.network.e,self.threads,rule.config.maximum_weight,
    ...
    def observe(self, rule, activity, reward):
        ...
        count=self.sparse(len(rule.keys),len(incoming),int(cfg.visual_eligibility=='forecast-causal-v1'),self.threads,
