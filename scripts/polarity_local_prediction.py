"""Online local event learning from history unit and observed raw polarity."""
...
OUT = Path('docs/experiments/2026-09-25-polarity-local-prediction-results.json')
NAMES = ('archived', 'unsplit', 'learned_split',
         'random_split', 'trace_reset')
...
def train_candidates(cases):
    ...
    models = dict(
        unsplit=LocalEventReadout(copy.deepcopy(codes['learned'])),
        learned_split=LocalEventReadout(copy.deepcopy(codes['learned']),
                                        polarity_split=True),
        random_split=LocalEventReadout(copy.deepcopy(codes['random']),
                                       polarity_split=True))
    ...
    models['trace_reset'] = copy.deepcopy(models['learned_split'])
