"""Preregistered frozen operating-point experiment for existing lamina feedback."""
import argparse
...
def main():
    ...
    parser.add_argument('--config', default='configs/feedback-calibration-v1.json')
    parser.add_argument('--output', default='runs/feedback-calibration-v1.json')
    ...
    labels = ('L1','L2','L3','C2','C3','L4','Lawf1','T4','T5')
    masks = {label:torch.tensor([t == label or (label in ('T4','T5') and t.startswith(label)) for t in types]) for label in labels}
    rows = []
    for rest in spec['centrifugal_rest_currents']:
        params = {t:dict(p) for t,p in source.network.config.class_parameters.items()}
        params.update(spec['other_classes'])
        params.update({t:{'rest_current':rest} for t in spec['centrifugal_classes']})
        ...
        qualifies = stable and all(max(populations[t]['stimulus_counts'][:2]) > 0 and
            max(populations[t]['stimulus_changed_neurons']) > 0 for t in labels if t not in ('T4','T5'))
