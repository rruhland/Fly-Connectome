...
- [ ] Next: audit whether Tm4's own subthreshold local current persists at those unsupported fast-holdout events. Only if it does, test a single opt-in current-driven Tm4 release profile on measured edges with moving/static/blank and position/speed controls. Then design a local afferent credit rule that preserves temporal order and tests actual anticipation, without production promotion until the user approves a concrete revision.
