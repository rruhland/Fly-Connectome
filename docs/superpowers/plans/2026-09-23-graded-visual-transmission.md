...
- [ ] Next: return to M1A's learning objective. Audit whether existing local predictive eligibility at T5 can forecast its own frozen motion-selective gate/spikes at the frame horizon, across position and speed, before designing a class-local credit rule. Keep CT1 shadows experimental and do not sweep their gain/lag/nearest-neighbor parameters or promote them into production.
