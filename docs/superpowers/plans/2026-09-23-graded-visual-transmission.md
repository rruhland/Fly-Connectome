...
- [ ] Test one opt-in current-driven Tm4 release profile on measured edges with moving/static/blank and position/speed controls. Require improved fast-motion arm coverage without increased static false alarms. Then design a local afferent credit rule that preserves temporal order and tests actual anticipation, without production promotion until the user approves a concrete revision.
