...
- [x] Compare Tm4/Tm9 afferent history with the same future T5 gate samples. Total upstream support is 94.7–99.7% versus 20.7–51.2% recurrence, but Tm4 alone misses the preregistered 50% requirement in one fast holdout (46.9%); static responses remain a specificity risk.
- [x] Audit Tm4's own subthreshold local current at unsupported fast-holdout events. The preregistered screen passes for T5d/up, but static-bar responses are stronger than moving responses; do not infer selectivity from current presence alone.
- [x] Test one opt-in current-driven, spike-silent Tm4 supplement on measured Tm4→T5 edges with moving/static/blank and position/speed controls. Fast T5d/up arm coverage rises 48.7%→95.2%, but T5c at y=16/speed 1 narrowly fails the fixed 90%-of-baseline moving/static criterion. Keep the prototype opt-in.
- [x] Preregister and test one faster (50 ms) source-local Tm4 release baseline against the original and two new static/moving position/speed controls. It passes the frozen T5 feature gate: fast T5d/up arm support rises 48.7%→93.9%, direction signs transfer, and all moving/static and blank limits hold. Keep the profile opt-in.
- [x] Preflight an independent future-arrival target before designing credit. Raw T5 feedforward-arrival events occur on 24–28% of bright-blank samples and yield only 0.51–0.61 preferred-motion AUC; do not train on the raw threshold.
- [x] Test exact paired blank-subtracted future afferent arrivals as an offline upper bound. Preferred-motion AUC remains 0.52–0.64 and static event prevalence remains high; do not implement a causal baseline for this failed target.
- [x] Test a local next-frame T5 future-spike target against persistence and static/blank. It is motion-specific but order-state AUC is 0.52–0.68 and rarely beats persistence enough; do not train on the one-frame target.
- [x] Run a fixed 8/16/32-tick future-T5-spike horizon audit. AUC declines with longer horizons in every position/speed case; stop window shifts for the single-cell order target.
- [ ] Reassess the M1A learning objective around measured retinotopic population sequences. Use the prior LPi/VS route and sign audits, predefine a spatially shifted downstream event target, and test causal route/timing capacity before opt-in local credit. Seek user approval before production promotion.
