...
- [ ] Test paired blank-subtracted stimulus-driven arrivals as an offline capacity diagnostic, then a causal cell-local baseline if warranted. Only after an independent target passes, design local afferent credit and test next-frame anticipation against frozen, static, and withheld controls. Seek user approval before production promotion.
