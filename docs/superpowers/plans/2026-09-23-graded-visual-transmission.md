...
- [ ] Preregister and test a source-local temporal-selectivity mechanism or independently selected release dynamics against new static/moving controls. Only after a frozen feature gate passes, design a local afferent credit rule preserving temporal order and test actual anticipation; seek user approval before production promotion.
